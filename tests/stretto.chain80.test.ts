...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert.equal(options.targetChainLength, 80, 'Expected chain size target to be 80');
    ...
    if (report.results.length > 0) {
      ...
      assert.ok(leader.entries.length >= 3, 'Expected non-trivial chain entries when results are present');
      assert.equal(leader.entries[0].startBeat, 0, 'Expected chain to start at beat 0');
    }

    console.log('stretto.chain80.test passed', {
      subjectBeats: 12,
...
run().catch((error) => {
  console.error('stretto.chain80.test failed');
  console.error(error);
  process.exit(1);
});
