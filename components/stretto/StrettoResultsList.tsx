...
export default function StrettoResultsList({ results, selectedId, onSelect, voiceNames }: StrettoResultsListProps) {
    ...
    const scoredValidResults = results.filter(
        (result): result is StrettoChainResult & { score: number } => result.isValid === true && Number.isFinite(result.score)
    );

    if (scoredValidResults.length === 0) {
        return <div className="text-center text-gray-500 py-10">No chains found yet. Run the search to begin.</div>;
    }

    const sortedResults = [...scoredValidResults].sort((a, b) => b.score - a.score);
